...
bookForm.addEventListener("submit", (e) => {
  ...
  const config = {
    // composing the config object that will set option on our POST fetch
    method: "POST",
    headers: {
  ...
  fetch("http://localhost:3000/books", config)
    ...
    .catch(renderError); // shorthand for (error => renderError(error))
  e.target.reset();
  toggleBookForm();
});
...
function handleStoreSubmit(e) {
  ...
  ///////////////////////////////
  // const newOption = addSelectOptionForStore(newStore)
  // postJSON('http://localhost:3000/stores', newStore)
  //   .then(dbStore => newOption.value = dbStore.id) // json-server assigns new id automatically

  // fetch + pessimistic rendering
  ///////////////////////////////////
  // postJSON("http://localhost:3000/stores", newStore)
  //   .then((dbStore) => addSelectOptionForStore(dbStore))
  //   .catch(renderError);
